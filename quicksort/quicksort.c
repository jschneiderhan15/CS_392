...
static int lomuto(void *array, int left, int right, size_t elem_sz,
                  int (*comp) (const void*, const void*)) {
    char *p = (char *)array + (elem_sz * left);
    char *newarray = (char*) array;
    size_t s = left;
    for(size_t i = left + 1; i <= right; i++){
        if(comp(newarray + (elem_sz * i), p) < 0){
            s++;
            swap(newarray + (elem_sz * s), newarray + (elem_sz * i), elem_sz);
        }
    }
    if(left != s){
    	swap(newarray + (elem_sz * left), newarray + (elem_sz * s), elem_sz);
    }
    return s;
}
